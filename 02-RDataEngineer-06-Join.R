...
merge(x, y, by = c("A1", "A2"),all.y = TRUE)#不多贅述
merge(x, y, by = c("A1", "A2"),all = TRUE)#全部保留
#by參數一定要x,y都有，如果沒有設定by參數則預設挑出相同的colname來比較
...
inner_join(x,y,by = c("A1","A2")); merge(x, y, by = c("A1", "A2"))
#請比較這兩者的不同，inner_join不會對資料做排列而merge會
#在處理資料的時候，有時NA值不是我們樂見的，或是不允許資料隨意更動
#依照不同需求而使用不同函數
...
setdiff(x,y) #比較兩者不同的元素，輸出x中不同的元素
...
#cut()會依照breadk中進行區分，觀察規律
#拿3做例子 2 < 3 <= 3，3被分在(2,3]這一組，所以0是位於(-Inf,0]這一組
#而不是分在(0,1]這組，所以我們要將最小分級往後扣一點，0才能夠被分級
